...
int main(int argc, char** argv) {
  ...
  const char filename[] = "bfly/models/floor.tflite";
  ...
  std::vector<float> out4(16);

  const TfLiteTensor* output_tensor = TfLiteInterpreterGetOutputTensor(interpreter, 0);
  TfLiteTensorCopyToBuffer(output_tensor, out4.data(), out4.size() * sizeof(float));


  std::cout << "out4.size() " << out4.size() << " sizeof(float) = " << sizeof(float) << std::endl;
